...
class StudentApplication:
    ...
    def insertStudent(self):
        ...
        try:
            ...
            self.table_user_interface.print_message("Neplatné jméno: Jméno musí obsahovat sadu maximálně 20 písmen.")
        except LastNameValueError:
            self.table_user_interface.print_message("Neplatné přijmení: Přijmení musí obsahovat sadu maximálně 20 písmen.")
        except UserNameValueError:
            self.table_user_interface.print_message("Neplatné uživatelské jméno: username musí obsahovat sadu maximálně 20 písmen.")
        else:
            try:
    ...
    def deleteStudent(self):
        ...
        self.table_user_interface.print_message(len(user_name))
        if 0 > len(user_name) and len(user_name) <= 20:
            try:
    ...
    def changeStudentClass(self):
        ...
        try:
            ...
            self.table_user_interface.print_message("Požadavek byl poslán do databáze.")
        except DatabaseError:
        ...
        try:
            ...
            if len(students) != 0:
                for student in students:
                    print(f" Student: {student[0]} {student[1]}, s číslem v třídním výkazu {student[2]}")
            else:
                self.table_user_interface.print_message("Třídat nebyla nalezena")
        except DatabaseError:
